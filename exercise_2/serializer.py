...
class JsonSerializer:
    ...
    def deserialize_invoice(cls, invoice_json: str, masking: bool=False) -> Invoice:
        ...
        customer = cls._parse_customer(customer_data=get_required_field(data, "customer"), masking=masking)
        line_items = cls._parse_line_items(line_items_data=get_required_field(data,"line_items"))

        return Invoice(
            invoice_id = get_required_field(data, "invoice_id"),
            issued_date = safe_cast_to_datetime(get_required_field(data, "issued_date"), "issued_date"),
            due_date = safe_cast_to_datetime(get_required_field(data, "due_date"), "due_date"),
    ...
    @staticmethod
    def _parse_customer(customer_data: dict[str, Any], masking: bool=False) -> Customer:
    ...
    def _parse_line_items(line_items_data: dict[str, Any]) -> list[LineItems]:
        ...
        for item in line_items_data:
            line_items.append(LineItems(description= get_required_field(item, "description"),
                               quantity=validate_is_positive(safe_cast_to_int(get_required_field(item, "quantity"), "quantity")),
                               unit_price=safe_cast_to_float(get_required_field(item, "unit_price"), "unit_price"),
